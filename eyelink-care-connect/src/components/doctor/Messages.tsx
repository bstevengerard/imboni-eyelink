...
import { CheckCheck, Check, MessageSquare, Paperclip, Search, Send, Plus } from 'lucide-react';
...
export default function DoctorMessages() {
  ...
  const [newConversationPatient, setNewConversationPatient] = useState<string>('');
  ...
  useEffect(() => {
    if (!selectedConversation?.patient_id) return;
    
    const cleanup = onPresenceUpdate?.((data) => {
      if (String(data?.userId) === String(selectedConversation.patient_id)) {
        setOtherUserOnline(data.online);
      }
    });
    
    return () => cleanup?.();
  }, [onPresenceUpdate, selectedConversation?.patient_id]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get<ConversationRow[]>('/api/doctor/messages/conversations');
        if (cancelled) return;
  ...
  const searchPatients = async () => {
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        setPatients(res.data.map((p: any) => ({ id: String(p._id ?? p.id), name: p.name ?? '' })));
      }
    } catch {
      setPatients([]);
    }
  ...
  const handleCreateConversation = async () => {
    if (!newConversationPatient) return;
    try {
      const res = await api.post('/api/doctor/messages/conversations', { patient_id: newConversationPatient });
      if (res.success && res.data?.id) {
        const convId = String(res.data.id);
        const patient = patients.find((p) => String(p.id) === String(newConversationPatient));
        if (patient) {
          setConversations((prev) => [
            {
              id: convId,
              patient_id: newConversationPatient,
...
useEffect(() => {
     ...
     (async () => {
       ...
       try {
         const res = await api.get<any[]>(`/api/doctor/messages/conversations/${selectedConversationId}/messages`);
         if (cancelled) return;
  ...
  useEffect(() => {
    if (selectedConversation?.patient_id) {
      api.get(`/api/presence/${selectedConversation.patient_id}`)
        .then((res) => {
      ...
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        ...
        <Dialog open={isNewConversationOpen} onOpenChange={setIsNewConversationOpen}>
          <DialogTrigger asChild>
            <Button variant="outline" size="sm" onClick={searchPatients}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <Select value={newConversationPatient} onValueChange={setNewConversationPatient}>
                <SelectTrigger>
              ...
              <Button onClick={handleCreateConversation} disabled={!newConversationPatient} className="w-full">
                Start Conversation
              </Button>
            </div>
          </DialogContent>
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center shrink-0">
                      <span className="text-sm font-semibold text-accent">{initials(conv.patient_name)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <h4 className="font-semibold text-foreground text-sm truncate">{conv.patient_name}</h4>
          ...
          {selectedConversation ? (
                ...
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center">
                      <span className="text-sm font-semibold text-accent">{initials(selectedConversation.patient_name)}</span>
                    </div>
                  ...
                  <div>
                    <h4 className="font-semibold text-foreground">{selectedConversation.patient_name}</h4>
                          ...
                          <div className="max-w-[70%] group">
                            ...
                            >
                              ...
                              <div className={`flex items-center gap-1 mt-1 ${isMe ? 'justify-end' : ''}`}>
                                <span
                                  className={`text-[10px] ${
                                    isMe ? 'text-accent-foreground/70' : 'text-muted-foreground/70'
                                  }`}
                                >
            ...
            <div className="flex-1 flex items-center justify-center text-center min-h-0">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Select a patient conversation from the list to start messaging</p>
              </div>
            </div>
          )}
        </div>
